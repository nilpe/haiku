...
extern status_t _kern_get_safemode_option(const char *parameter, char *buffer,
                                          size_t *_bufferSize);
...
extern status_t _kern_event_queue_select(int queue,
                                         struct event_wait_info *userInfos,
                                         int numInfos);
extern ssize_t _kern_event_queue_wait(int queue, struct event_wait_info *infos,
                                      int numInfos, uint32 flags,
                                      bigtime_t timeout);

/* user mutex functions */
extern status_t _kern_mutex_lock(int32 *mutex, const char *name, uint32 flags,
                                 bigtime_t timeout);
...
extern status_t _kern_realtime_sem_open(const char *name, int openFlagsOrShared,
                                        mode_t mode, uint32 semCount,
                                        struct _sem_t *userSem,
                                        struct _sem_t **_usedUserSem);
...
extern ssize_t _kern_xsi_msgrcv(int messageQueueID, void *messagePointer,
                                size_t messageSize, long messageType,
                                int messageFlags);
extern int _kern_xsi_msgsnd(int messageQueueID, const void *messagePointer,
...
extern thread_id _kern_load_image(const char *const *flatArgs,
                                  ...
                                  int32 envCount, int32 priority, uint32 flags,
...
extern thread_id
_kern_spawn_thread(struct thread_creation_attributes *attributes);
...
extern int32 _kern_receive_data(thread_id *_sender, void *buffer,
                                size_t bufferSize);
extern int64
_kern_restore_signal_frame(struct signal_frame_data *signalFrameData);
...
extern status_t _kern_get_next_thread_info(team_id team, int32 *cookie,
                                           thread_info *info);
extern status_t _kern_get_team_info(team_id id, team_info *info, size_t size);
...
extern status_t _kern_get_thread_affinity(thread_id id, void *userMask,
                                          size_t size);
extern status_t _kern_set_thread_affinity(thread_id id, const void *userMask,
                                          size_t size);
...
extern status_t _kern_setregid(gid_t rgid, gid_t egid, bool setAllIfPrivileged);
extern status_t _kern_setreuid(uid_t ruid, uid_t euid, bool setAllIfPrivileged);
...
extern status_t _kern_entry_ref_to_path(dev_t device, ino_t inode,
                                        ...
                                        size_t pathLength);
extern status_t _kern_normalize_path(const char *userPath, bool traverseLink,
                                     char *buffer);
extern int _kern_open_entry_ref(dev_t device, ino_t inode, const char *name,
...
extern int _kern_open_attr_dir(int fd, const char *path, bool traverseLeafLink);
...
extern status_t _kern_create_index(dev_t device, const char *name, uint32 type,
                                   uint32 flags);
...
extern status_t _kern_write_stat(int fd, const char *path, bool traverseLink,
                                 const struct stat *stat, size_t statSize,
                                 int statMask);
...
extern ssize_t _kern_recv(int socket, void *data, size_t length, int flags);
extern ssize_t _kern_recvfrom(int socket, void *data, size_t length, int flags,
                              struct sockaddr *address,
                              socklen_t *_addressLength);
extern ssize_t _kern_recvmsg(int socket, struct msghdr *message, int flags);
...
extern ssize_t _kern_sendmsg(int socket, const struct msghdr *message,
                             int flags);
extern status_t _kern_getsockopt(int socket, int level, int option, void *value,
                                 socklen_t *_length);
...
extern int32
_kern_create_timer(clockid_t clockID, thread_id threadID, uint32 flags,
                   const struct sigevent *event,
                   const struct thread_creation_attributes *threadAttributes);
...
extern area_id _kern_map_file(const char *name, void **address,
                              ...
                              uint32 protection, uint32 mapping,
...
extern status_t _kern_memory_advice(void *address, size_t size, uint32 advice);

extern status_t _kern_get_memory_properties(team_id teamID, const void *address,
...
extern status_t _kern_writev_port_etc(port_id id, int32 msgCode,
                                      const struct iovec *msgVecs,
                                      size_t vecCount, size_t bufferSize,
                                      uint32 flags, bigtime_t timeout);
extern status_t _kern_get_port_message_info_etc(port_id port,
                                                port_message_info *info,
                                                size_t infoSize, uint32 flags,
                                                bigtime_t timeout);
...
extern port_id _kern_install_team_debugger(team_id team, port_id debuggerPort);
...
extern status_t _kern_clear_debugger_breakpoint(void *address, bool watchpoint);

extern status_t
_kern_system_profiler_start(struct system_profiler_parameters *parameters);
...
extern status_t
_kern_system_profiler_recorded(struct system_profiler_parameters *parameters);
...
extern status_t _kern_get_file_disk_device_path(partition_id id, char *buffer,
                                                size_t bufferSize);
...
extern status_t _kern_move_partition(partition_id partitionID,
                                     ...
                                     int32 *descendantChangeCounters,
