...
#define _POSIX_MONOTONIC_CLOCK                                                 \
  (200809L) /* aio_suspend() is missing in Haiku1 */
...
extern int linkat(int toFD, const char *toPath, int pathFD, const char *path,
                  int flag);
...
#endif
#else
inline int ioctl(int fd, unsigned long op, void *argument = NULL,
                 size_t size = 0) {
...
extern int getgrouplist(const char *user, gid_t baseGroup, gid_t *groupList,
                        int *groupCount);
