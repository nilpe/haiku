...
static generic_syscall *find_generic_syscall(const char *subsystem) {
...
status_t _user_generic_syscall(const char *userSubsystem, uint32 function,
  ...
  while (syscall != NULL) {
    ...
    if (syscall->valid) {
      ...
      status_t status = syscall->hook(subsystem, function, buffer, bufferSize);
...
int _user_is_computer_on(void) { return 1; }

//	#pragma mark -

int32 syscall_dispatcher(uint32 callIndex, void *args, uint64 *_returnValue) {
  bigtime_t startTime;

  //	dprintf("syscall_dispatcher: thread 0x%x call 0x%x, arg0 0x%x, arg1 0x%x
  // arg2 0x%x arg3 0x%x arg4 0x%x\n", 		thread_get_current_thread_id(),
...
status_t generic_syscall_init(void) {
  ...
  add_debugger_command_etc(
      "straced", &dump_syscall_tracing, "Dump recorded syscall trace entries",
...
static const char *get_syscall_name(uint32 syscall) {
...
public:
  PreSyscall(uint32 syscall, const void *parameters)
      : fSyscall(syscall), fParameters(NULL) {
    if (syscall < (uint32)kSyscallCount) {
      fParameters = alloc_tracing_buffer_memcpy(
          parameters, kSyscallInfos[syscall].parameter_size, false);
      ...
      if (fParameters != NULL && syscall != SYSCALL_KTRACE_OUTPUT) {
        ...
        for (int i = 0; i < syscallInfo.parameter_count; i++) {
          const syscall_parameter_info &paramInfo = syscallInfo.parameters[i];
          ...
          if (stringIndex < MAX_PARAM_STRINGS) {
            fParameterStrings[stringIndex++] =
                alloc_tracing_buffer_strcpy(*(const char **)data, 64, true);
          }
        }
      }
    }
...
public:
  PostSyscall(uint32 syscall, uint64 returnValue)
      : fSyscall(syscall), fReturnValue(returnValue) {
    Initialized();
#if 0
			if (syscall < (uint32)kSyscallCount
  ...
  virtual void AddDump(TraceOutput &out) {
    out.Print("syscall post: %s() -> %#" B_PRIx64, get_syscall_name(fSyscall),
              fReturnValue);
  }
...
void trace_pre_syscall(uint32 syscallNumber, const void *parameters) {
...
void trace_post_syscall(int syscallNumber, uint64 returnValue) {
#if SYSCALL_TRACING_IGNORE_KTRACE_OUTPUT
  if (syscallNumber != SYSCALL_KTRACE_OUTPUT)
#endif
  {
...
public:
  ...
  bool HitThreadLimit() const { return fHitThreadLimit; }

  int Direction() const { return fDirection; }

private:
  enum { MAX_PENDING_THREADS = 32 };
